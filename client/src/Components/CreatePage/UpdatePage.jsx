...
function UpdatePage() {
  ...
  useEffect(() => {
    const fetchPage = async () => {
      ...
      const sanitizedData = {
        ...listingData,
        name: typeof listingData.name === 'string' ? { en: listingData.name, ar: listingData.name } : (listingData.name || { en: '', ar: '' }),
        description: typeof listingData.description === 'string' ? { en: listingData.description, ar: listingData.description } : (listingData.description || { en: '', ar: '' }),
        address: typeof listingData.address === 'string' ? { en: listingData.address, ar: listingData.address } : (listingData.address || { en: '', ar: '' }),
        ...
        imagePlans:      Array.isArray(listingData.imagePlans)      ? listingData.imagePlans      : [],
        imageApartments: Array.isArray(listingData.imageApartments) ? listingData.imageApartments : [],
      };
  ...
  return (
    <div dir="rtl" className="min-h-screen py-8 px-4 lg:px-12" style={{ background: '#faf8f4' }}>
      <div className="mb-8 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-black" style={{ color: '#12283C' }}>تعديل المشروع</h1>
          <p className="text-xs mt-0.5" style={{ color: '#6b5e3e' }}>تحديث بيانات المشروع العقاري</p>
        </div>
      </div>
          ...
          <div className="space-y-6">
            ...
            <div className="pt-4" style={{ borderTop: '1px solid rgba(138,105,36,0.1)' }}>
              <button type="submit" disabled={loading || uploading || uploadingPlan}
                ...
                {loading ? <><TbLoaderQuarter className="animate-spin" size={18} /> جارٍ الحفظ...</> : 'حفظ التغييرات'}
              </button>
              {error && (
